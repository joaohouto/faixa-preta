...
export const nextMonth = (day, month, year) => {
	if (month > 11) return [day, "01", tratarNumero(+year + 1)];

	return [day, tratarNumero(+month + 1), year];
};

export const previousMonth = (day, month, year) => {
	if (month < 2) return [day, "12", tratarNumero(year - 1)];
...
export const gerarCalendario = (anoAtual, mesAtual) => {
	...
	if (diaUmSemana > 0) {
		for (let i = diaUmSemana - 1; i >= 0; i--) {
			...
			semana.push(
				qtdDiasMesPassado - i + "/" + tratarNumero(mesAtual) + "/" + anoAtual
			);
		}
	}

	for (
		let contagemDiasMes = 1;
		contagemDiasMes <= qtdDiasMeses[mesAtual];
		contagemDiasMes++
	) {
		if (semana.length < 7) {
			semana.push(
				tratarNumero(contagemDiasMes) +
					"/" +
					tratarNumero(mesAtual + 1) +
					"/" +
					anoAtual
			);
		} else {
	...
	for (
		let contagemDiasMes = 1;
		contagemDiasMes <= semana.length + 1;
		contagemDiasMes++
	) {
		if (semana.length < 7) {
			semana.push(
