...
export default class ActivityEdit extends Component {
    ...
    saveEdit = (e) => {
        ...
        api.put(`/activities/${this.state.activity._id}`, this.state.activity)
        .then(response => {
            alert("Ok!");
    ...
    loadData = async () => {
        ...
        const id = this.props.match.params.actv_id;
        const response = await api.get('/activities/' + id);

        this.setState({ activity: response.data, activityMoves: response.data.moves });
    }
            ...
            <div>
                ...
                <header className="jumbotron">
                    <div className="container">
                        <h2><UseAnimations animationKey="settings" size={40} style={{ position: 'relative', top: -3 }} /> Editar atividade</h2>
                    </div>
                </header>
...
<div className="form-row">
    <div className="form-group col-md-6">
        <label>Nome</label>
        <input value={activity.name} onChange={(e) => this.handleNameChange(e.target.value)} type="text" className="form-control" required />
    </div>

    <div className="form-group col-md-6">
        <label>Tags</label>
        <input value={activity.tags} onChange={(e) => this.handleTagsChange(e.target.value)} required type="text" className="form-control" placeholder="Ex: Tag 1,Tag 2" />
    </div>
</div>

<div className="form-group">
    <label>Detalhes</label>
    <textarea value={activity.details} onChange={(e) => this.handleDetailsChange(e.target.value)} required className="form-control" rows="3"></textarea>
</div>

<div className="form-group">
    <label>Imagem</label>
    <input value={activity.image} onChange={(e) => this.handleImageChange(e.target.value)} type="text" required className="form-control" placeholder="Ex: URL" />
</div>
